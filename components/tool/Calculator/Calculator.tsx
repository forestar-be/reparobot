...
const CostCalculator = (): JSX.Element => {
  ...
  const calculateEstimate = () => {
    ...
    cost += baseCost;
    breakdown.push(
      `Coût de base pour une pelouse de ${lawnSize} m² : ${baseCost.toFixed(2)} €`,
    );
    explanationTexts.push(
      `La taille de votre pelouse est de ${lawnSize} m². Plus la pelouse est grande, plus le robot tondeuse doit travailler, ce qui augmente le coût de maintenance de base.`,
    );
    ...
    if (answers.usageFrequency === 'Quotidienne') {
      ...
      explanationTexts.push(
        "Une utilisation quotidienne augmente l'usure du robot tondeuse, nécessitant plus de maintenance.",
      );
    } else if (answers.usageFrequency === 'Hebdomadaire') {
      usageMultiplier = 1.0;
      breakdown.push(
        'Ajustement pour utilisation hebdomadaire : aucun ajustement',
      );
      explanationTexts.push(
        'Une utilisation hebdomadaire maintient une usure régulière, sans impact supplémentaire sur les coûts de maintenance.',
      );
    } else if (answers.usageFrequency === 'Mensuelle') {
      usageMultiplier = 0.8;
      breakdown.push('Ajustement pour utilisation mensuelle : -20%');
      explanationTexts.push(
        "Une utilisation mensuelle réduit l'usure du robot tondeuse, diminuant ainsi les besoins en maintenance.",
      );
    }
    ...
    if (answers.terrainType === 'Accidenté') {
      ...
      explanationTexts.push(
        "Un terrain accidenté peut causer plus de stress et d'usure sur le robot tondeuse, augmentant les coûts de réparation et de maintenance.",
      );
    } else if (answers.terrainType === 'En pente') {
      cost += 30;
      breakdown.push('Coût supplémentaire pour terrain en pente : +30 €');
      explanationTexts.push(
        'Un terrain en pente exige plus de puissance et de résistance de la part du robot tondeuse, ce qui peut entraîner des coûts de maintenance supplémentaires.',
      );
    } else {
      breakdown.push('Terrain plat : aucun coût supplémentaire');
      explanationTexts.push(
        'Un terrain plat permet un fonctionnement optimal du robot tondeuse sans usure additionnelle.',
      );
    }
    ...
    if (answers.technicalIssues === 'Oui') {
      ...
      explanationTexts.push(
        'Des problèmes techniques indiquent des réparations ou des remplacements de pièces, augmentant ainsi les coûts de maintenance.',
      );
    } else {
      breakdown.push(
        'Aucun problème technique signalé : aucun coût supplémentaire',
      );
      explanationTexts.push(
        'Aucun problème technique détecté, ce qui signifie moins de réparations et donc des coûts de maintenance réduits.',
      );
    }
    ...
    if (answers.maintenanceHistory === 'Oui') {
      ...
      explanationTexts.push(
        'Un entretien régulier prolonge la durée de vie du robot tondeuse et réduit les besoins en réparations majeures.',
      );
    } else {
      breakdown.push("Pas d'entretien régulier : aucune réduction du coût");
      explanationTexts.push(
        "L'absence d'entretien régulier peut entraîner une usure plus rapide et des coûts de maintenance plus élevés.",
      );
    }
    ...
    if (accessoriesCost > 0) {
      breakdown.push(
        `Coût pour les accessoires (${answers.accessories.join(', ')}): +${accessoriesCost.toFixed(2)} €`,
      );
      explanationTexts.push(
        `Vous utilisez les accessoires suivants : ${answers.accessories.join(', ')}. Chaque accessoire ajoute 20 € aux coûts de maintenance.`,
      );
    } else {
      breakdown.push('Aucun accessoire utilisé : aucun coût supplémentaire');
      explanationTexts.push(
        "Vous n'utilisez aucun accessoire, ce qui évite des coûts de maintenance additionnels.",
      );
    }
      ...
      <div className="quiz-step">
        {step === 1 && (
          <>
            ...
            <div className="navigation-buttons">
              ...
              <button disabled={!answers.brand} onClick={handleNext}>
                Suivant
              </button>
            </div>
          </>
        ...
        {step === 2 && (
          <>
            ...
            <div className="navigation-buttons">
              ...
              <button disabled={!answers.model} onClick={handleNext}>
                Suivant
              </button>
            </div>
          </>
        ...
        {step === 3 && (
          <>
            ...
            <input
              ...
              onChange={(e: any) =>
                handleInputChange('lawnSize', e.target.value)
              }
              min="0"
        ...
        {step === 4 && (
          <>
            ...
            <div className="navigation-buttons">
              <button onClick={handleBack}>Précédent</button>
              <button disabled={!answers.usageFrequency} onClick={handleNext}>
                Suivant
              </button>
            </div>
          </>
        ...
        {step === 5 && (
          <>
            ...
            <select
              ...
              onChange={(e: any) =>
                handleInputChange('terrainType', e.target.value)
              }
            >
            ...
            <div className="navigation-buttons">
              <button onClick={handleBack}>Précédent</button>
              <button disabled={!answers.terrainType} onClick={handleNext}>
                Suivant
              </button>
            </div>
          </>
        ...
        {step === 6 && (
          <>
            ...
            <p>
              Avez-vous rencontré des problèmes techniques au cours de l'année
              passée ?
            </p>
            <select
            ...
            <div className="navigation-buttons">
              <button onClick={handleBack}>Précédent</button>
              <button disabled={!answers.technicalIssues} onClick={handleNext}>
                Suivant
              </button>
            </div>
          </>
        ...
        {step === 7 && (
          <>
            ...
            <p>
              Avez-vous effectué un entretien régulier de votre robot tondeuse ?
            </p>
            <select
        ...
        {step === 8 && (
          <>
            ...
            <p>
              Utilisez-vous des accessoires avec votre robot tondeuse ?
            ...
            <div className="checkbox-group">
              ...
              />
              <label htmlFor="accessory-bluetooth">
            ...
            {answers.accessories.includes('Autre') && (
              <input
                ...
                onChange={(e: any) => {
                  const otherAccessories = e.target.value
                    .split(',')
        ...
        {step === 9 && estimatedCost !== null && (
          <>
            ...
            <p>
              Sur la base de vos réponses, voici une estimation détaillée du
              coût annuel d'entretien de votre robot tondeuse :
            </p>
            ...
            <div className="total-cost">
              <h2>
                Coût d'entretien annuel estimé : {estimatedCost.toFixed(2)} €
              </h2>
            </div>
            <h4>Explications :</h4>
            ...
            <p>
              Merci d'avoir utilisé notre simulateur de coût de maintenance !
            </p>
